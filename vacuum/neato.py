...
def setup_platform(hass, config, add_entities, discovery_info=None):
    ...
    for robot in hass.data[NEATO_ROBOTS]:
        ...
        controller[robot.name] = NeatoConnectedVacuum(hass, robot)
    _LOGGER.debug("Adding vacuums %s", dev)
    add_entities(dev, True)

    def neato_custom_cleaning_service(call):
        """Custom cleaning service that allows user to change options"""
        mode = call.data.get(ATTR_MODE)
        navigation = call.data.get(ATTR_NAVIGATION)
        category = call.data.get(ATTR_CATEGORY)
        name = call.data.get(ATTR_NAME)
        controller[name].neato_custom_cleaning(mode, navigation, category)
...
class NeatoConnectedVacuum(StateVacuumDevice):
    ...
    def neato_custom_cleaning(self, mode, navigation, category, **kwargs):
        """Custom cleaning service call"""
        self._clean_state = STATE_CLEANING
        self.robot.custom_cleaning(mode, navigation, category)

    def neato_zone_cleaning(self, boundary_id, **kwargs):
